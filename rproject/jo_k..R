...
data <- read.csv("C:\\Users\\hp\\Desktop\\rproject\\DATA1.csv", header = TRUE, sep = ",")
df=data.frame(data)
df
...
df$total_sqft=as.integer(df$total_sqft)
...
test_index <- createDataPartition(df$price, p = test_size, list = FALSE)
...
rf_model <- randomForest(price ~ location+total_sqft+bath+size1, data = train_data, ntree = 101)
...
accuracy_percentage <- mean(abs(test_data$price - rf_predictions) / test_data$price <= accuracy_threshold) * 100
